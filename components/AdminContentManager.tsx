...
type Item = { id:string; content_type:ContentType; slug:string; title:string; category:string; summary:string; content:string; materials:string[]; steps:string[]; published:boolean; sort_order:number; video_url?:string };

type FormState = { slug:string; title:string; category:string; summary:string; content:string; materials:string; steps:string; esTitle:string; esCategory:string; esSummary:string; esContent:string; esMaterials:string; esSteps:string; videoUrl:string; esVideoUrl:string; published:boolean; sort_order:number };
...
export default function AdminContentManager({ type }: { type: ContentType }) {
  ...
  async function load() {
    if(!supabase) return;
    const {data,error:e} = await supabase.from('content_items').select('id,content_type,slug,title,category,summary,content,materials,steps,published,sort_order,video_url').eq('content_type',type).order('sort_order',{ascending:true}).order('created_at',{ascending:true});
  ...
  async function loadTranslation(id:string){
    if(!supabase)return;
    const {data} = await supabase.from('content_translations').select('title,category,summary,content,materials,steps,video_url').eq('content_id',id).eq('language','es-LA').maybeSingle();
    setForm(prev=>({...prev,esTitle:data?.title||'',esCategory:data?.category||'',esSummary:data?.summary||'',esContent:data?.content||'',esMaterials:Array.isArray(data?.materials)?data.materials.join('\n'):'',esSteps:Array.isArray(data?.steps)?data.steps.join('\n'):'',esVideoUrl:data?.video_url||''}));
  }
  ...
  async function loadMedia(slug:string){
    ...
    const {data}=await supabase.from('media_assets').select('slot,path,filename').eq('bucket',BUCKET[type]).in('slot',slots);
  ...
  function onImage(event:ChangeEvent<HTMLInputElement>,which:'cover'|'inside'){
    ...
    if(file){const text=file.name.replace(/\.[^.]+$/,'').replace(/[-_]+/g,' ');if(which==='cover'&&!coverAlt)setCoverAlt(text);if(which==='inside'&&!insideAlt)setInsideAlt(text);}
  }
    ...
    <div className="content-editor">
      ...
      <form className="settings-form" onSubmit={save}>
        <div className="form-grid">
          ...
          {type==='blog'&&<>
            <div className="full" style={{borderTop:'1px solid rgba(0,0,0,.08)',paddingTop:'1rem',marginTop:'.5rem'}}><span className="eyebrow">LISTA DE MATERIAIS — PORTUGUÊS</span></div>
            <label className="full">Materiais (um por linha)<textarea rows={6} value={form.materials} onChange={e=>setForm({...form,materials:e.target.value})} placeholder={'Tecido\nManta acrílica\nLinha\nTesoura'}/></label>
          </>}
          ...
          {type==='blog'&&<label className="full">Lista de materiales (ES) — uno por línea<textarea rows={6} value={form.esMaterials} onChange={e=>setForm({...form,esMaterials:e.target.value})} placeholder={'Tela\nManta acrílica\nHilo\nTijeras'}/></label>}
          ...
          <label className="full">Passo a passo (um por linha)<textarea rows={7} value={form.steps} onChange={e=>setForm({...form,steps:e.target.value})} placeholder={'Escolhendo os materiais\nCortando as peças\nMontando a peça\nFinalizando o acabamento'}/></label>
          <label className="full">Paso a paso (uno por línea) (ES)<textarea rows={7} value={form.esSteps} onChange={e=>setForm({...form,esSteps:e.target.value})} placeholder={'Eligiendo los materiales\nCortando las piezas\nMontando la pieza\nTerminando el acabado'}/></label>
          <label>Vídeo YouTube (PT)<input value={form.videoUrl} onChange={e=>setForm({...form,videoUrl:e.target.value})} placeholder="URL do vídeo"/></label>
          <label>Vídeo YouTube (ES)<input value={form.esVideoUrl} onChange={e=>setForm({...form,esVideoUrl:e.target.value})} placeholder="URL del vídeo"/></label>
