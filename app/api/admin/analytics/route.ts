...
export async function GET(request: Request) {
  ...
  if (!response.ok || json.errors) return NextResponse.json({ error: 'Falha ao consultar o Cloudflare Analytics.', details: json.errors || null }, { status: 502 });
  ...
  const weighted = (field: string) => {
    ...
    const value = vitals.reduce((sum: number, row: any) => sum + (row.count || 0) * (row.quantiles?.[field] ?? -1), 0) / samples;
