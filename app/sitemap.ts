...
const courses = ['feltro-criacoes-com-amor','patchwork-do-basico','arte-em-madeira','scrapbook-memorias'];
const projects = ['feltro-criacoes-com-amor','bolsa-patchwork','caixa-decorativa','album-de-memorias'];
const posts = ['cestinho-de-tecido','como-escolher-linhas','pintura-em-madeira','flores-de-feltro'];
...
export default function sitemap(): MetadataRoute.Sitemap {
  ...
  return [
    ...
    ...courses.map(slug => ({ url: `${base}/cursos/${slug}`, changeFrequency: 'monthly' as const, priority: 0.7 })),
    ...projects.map(slug => ({ url: `${base}/projetos/${slug}`, changeFrequency: 'monthly' as const, priority: 0.7 })),
    ...posts.map(slug => ({ url: `${base}/blog/${slug}`, changeFrequency: 'monthly' as const, priority: 0.7 })),
  ];
}
