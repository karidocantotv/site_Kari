...
export default async function Home() {
  ...
  const youtubeVideos = Array.from({ length: 5 }, (_, index) => {
    ...
    return { title: settings[`youtube_video_title${suffix}`] || `Vídeo ${index + 1}`, videoId, enabled: settings[`youtube_video_enabled${suffix}`] !== 'false' && Boolean(videoId) };
  ...
  return <>
    ...
    <main>
      ...
      <section className="trust"><div className="container trust-grid"><div className="trust-item"><div className="trust-icon">♡</div><div><b>Feito com afeto</b><span>Projetos pensados para trazer beleza e significado aos pequenos detalhes da casa.</span></div></div><div className="trust-item"><div className="trust-icon">✂</div><div><b>Aprenda fazendo</b><span>Aulas e conteúdos claros para você criar no seu ritmo, com segurança e prazer.</span></div></div><div className="trust-item"><div className="trust-icon">⌂</div><div><b>Para a sua casa</b><span>Peças para decorar, organizar, renovar ambientes e presentear quem você ama.</span></div></div><div className="trust-item"><div className="trust-icon">✦</div><div><b>Experiência da Kari</b><span>Uma trajetória na televisão transformada em inspiração para criar com as próprias mãos.</span></div></div></div></section>

      {/* 1. BLOG — agora vem antes dos vídeos e cursos */}
      <section className="section alt" hidden={!blogEnabled}><div className="container"><div className="section-head"><span className="eyebrow">Conteúdo gratuito</span><h2 className="serif">{settings.blog_title || 'Dicas e passo a passo para a casa'}</h2><p>{settings.blog_description || 'Aprenda uma técnica, escolha seus materiais e encontre inspiração para o próximo projeto.'}</p></div><BlogCarousel posts={blogItems} media={blogMedia} locale="pt" /></div></section>
