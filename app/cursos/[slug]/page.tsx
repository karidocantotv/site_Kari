...
export function generateMetadata({params}:{params:{slug:string}}){const c=courses[params.slug];return c?{title:c.title,description:c.desc}:{title:'Curso'}}
export default function Curso({params}:{params:{slug:string}}){const c=courses[params.slug];if(!c)notFound();return <><section className="pageHero"><span className="eyebrow">Curso online · {c.tag}</span><h1 className="serif">{c.title}</h1><p>{c.desc}</p></section><section className="article"><img src={c.image} alt={c.title}/><h2>O que você vai encontrar</h2><ul><li>Aulas organizadas para acompanhar no seu ritmo.</li><li>Orientações sobre materiais e preparação.</li><li>Detalhes de execução e acabamento.</li><li>Projeto pensado para colocar o aprendizado em prática.</li></ul><h2>Aprenda fazendo</h2><p>O objetivo é que você entenda o processo e ganhe confiança para criar outras peças a partir do que aprendeu.</p></section><section className="section alt"><div className="container"><LeadCapture courseSlug={params.slug} courseTitle={c.title} source="course-page" /></div></section></>}
